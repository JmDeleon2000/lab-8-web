...
const animation=(current, animpair, twoCards)=>
{
  animate(current)
  ...
  {sleep(300).then(()=>{  
    if (twoCards)
    if (animpair.stl === current.stl) 
    {
      setCards(cards.filter((card)=>(card.stl !== current.stl)))
      current.mode = false
      animpair.mode = false
      setFoundCards([...foundcards, current, animpair])
    }
    else setCards(cards.map((card) =>(card.id !== animpair.id ? card: {id: card.id, stl:card.stl, altstl:card.altstl, mode: !card.mode, animating: false})))
  })
  })
...
const changeMode = (id)=>
{
  ...
  const twoCards = JSON.stringify(pair) !== JSON.stringify({})
  const current = cards.filter((card)=>(card.id === id))[0]
  const animPair = pair
  if (twoCards)
  {
  ...
  pair = {}
    ...
    return (
      <>
        <div className = 'cont'>
          <h1>Moves {moves}</h1>
          {cards.length === 0 && <><h1>Great job!</h1><button className = 'retry' onClick = {reset} type='button'/></>}
